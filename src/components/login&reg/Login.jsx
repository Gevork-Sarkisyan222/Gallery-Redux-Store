...
function Login() {
  ...
  const onMakeLoginSuccsess = (e) => {
    e.preventDefault();

    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

    if (formData.email.match(emailRegex) && formData.password.length > 6) {
      alert('Вы успешно зашли на свой аккаунт ✔️✔️');
      navigate('/');
    } else {
      alert(
    ...
    <div className="Login-Main">
      <div className="login-wrapper">
        <form className="Login-Form-Main">
          <div class="container">
            <div class="heading">Войти в аккаунт</div>
            <form action="" class="form">
              ...
              <span class="forgot-password">
                <Link to="/Write/Phone-Number">регистрация</Link>
              </span>
              <input
                onClick={onMakeLoginSuccsess}
                class="login-button"
                type="submit"
                value="Войти"
              />
            ...
            <div class="social-account-container">
              <span class="title">Или войти с помошью</span>
            ...
            <span class="agreement">
              <a href="#">@Gallery-Store интернет магазин</a>
            </span>
          </div>
        </form>
      </div>
